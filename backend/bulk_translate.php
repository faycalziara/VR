...
<ul class="nav bg-white nav-pills nav-fill mb-2">
    ...
    <li class="nav-item">
        <a class="nav-link" data-toggle="pill" href="#poi_gallery_tab"><i class="fas fa-images"></i> <?php echo strtoupper(_("POI Gallery")); ?></a>
    </li>
    <li class="nav-item">
        <a class="nav-link" data-toggle="pill" href="#poi_staging_tab"><i class="fas fa-arrows-spin"></i> <?php echo strtoupper(_("Staging")); ?></a>
    </li>
</ul>
<div class="tab-content">
            ...
            <div class="col-md-12">
                <div data-table="svt_virtualtours_lang" class="card block_translate shadow mb-4">
                    ...
                    <div class="card-body">
                        ...
                        echo print_value_input('learning_modal_title',$virtual_tour,$array_input_lang,'svt_virtualtours_lang','text');
                        echo print_value_input('learning_modal_subtitle',$virtual_tour,$array_input_lang,'svt_virtualtours_lang','text');
                        echo print_value_input('learning_modal_description',$virtual_tour,$array_input_lang,'svt_virtualtours_lang','editor');
                        echo print_value_input('learning_modal_button',$virtual_tour,$array_input_lang,'svt_virtualtours_lang','text');
                        echo print_value_input('learning_summary_title',$virtual_tour,$array_input_lang,'svt_virtualtours_lang','text');
                        echo print_value_input('learning_summary_partial_title',$virtual_tour,$array_input_lang,'svt_virtualtours_lang','text');
                        echo print_value_input('learning_summary_global_title',$virtual_tour,$array_input_lang,'svt_virtualtours_lang','text');
                        echo print_value_input('learning_placeholder_email',$virtual_tour,$array_input_lang,'svt_virtualtours_lang','text');
                        ?>
                    </div>
                </div>
            </div>
                                ...
                                while ($row = $result->fetch_array(MYSQLI_ASSOC)) {
                                    ...
                                    if($result_lang) {
                                        if ($result_lang->num_rows > 0) {
                                            while($row_lang = $result_lang->fetch_array(MYSQLI_ASSOC)) {
                                                $language = $row_lang['language'];
                                                unset($row_lang['id_presentation']);
                                                unset($row_lang['language']);
                                                $array_input_lang[$language]=$row_lang;
                                            }
                                        }
    ...
    <div class="tab-pane" id="poi_gallery_tab">
        <div class="row">
            <div class="col-md-12">
                <div data-table="svt_poi_gallery_lang" class="card block_translate shadow mb-4">
                    <div class="card-header py-3">
                        <i class="fas fa-circle icon_check_translate"></i>&nbsp;&nbsp;<h6 class="m-0 font-weight-bold d-inline-block text-primary"><?php echo sprintf(_("%s of %s translated"),'<span class="count_translated">--</span>','<span class="count_total_translate">--</span>') ?></h6>
                        <?php if($deepl==1) : ?>
                            <button class="btn_bulk_translate btn btn-sm btn-primary ml-3 disabled <?php echo ($demo) ? 'disabled_d' : ''; ?>"><?php echo _("translate missing values"); ?>&nbsp;&nbsp;<i style="vertical-align:middle" class="fas fa-globe"></i></button>
                    ...
                    <div class="card-body">
                        <?php
                        $query = "SELECT * FROM svt_poi_gallery WHERE id_poi IN (SELECT id FROM svt_pois WHERE type='gallery' AND id_room IN (SELECT id FROM svt_rooms WHERE id_virtualtour=$id_virtualtour_sel));";
                        $result = $mysqli->query($query);
                        if($result) {
                            if ($result->num_rows > 0) {
                                while ($row = $result->fetch_array(MYSQLI_ASSOC)) {
                                    ...
                                    $gallery = $row;
                                    $array_input_lang = array();
                                    $query_lang = "SELECT * FROM svt_poi_gallery_lang WHERE id_poi_gallery=$id_gallery;";
                                    $result_lang = $mysqli->query($query_lang);
                                    if($result_lang) {
                                        if ($result_lang->num_rows > 0) {
                                            while($row_lang = $result_lang->fetch_array(MYSQLI_ASSOC)) {
                                                $language = $row_lang['language'];
                                                unset($row_lang['id_poi_gallery']);
                                                unset($row_lang['language']);
                                                $array_input_lang[$language]=$row_lang;
                                            }
                                        }
                                    }
                                    echo print_value_input('title',$gallery,$array_input_lang,'svt_poi_gallery_lang','text');
                                    echo print_value_input('description',$gallery,$array_input_lang,'svt_poi_gallery_lang','textarea');
                                }
                            }
                        }
                        ?>
    ...
    <div class="tab-pane" id="poi_staging_tab">
        <div class="row">
            <div class="col-md-12">
                <div data-table="svt_poi_staging_lang" class="card block_translate shadow mb-4">
                    <div class="card-header py-3">
                        <i class="fas fa-circle icon_check_translate"></i>&nbsp;&nbsp;<h6 class="m-0 font-weight-bold d-inline-block text-primary"><?php echo sprintf(_("%s of %s translated"),'<span class="count_translated">--</span>','<span class="count_total_translate">--</span>') ?></h6>
                        <?php if($deepl==1) : ?>
                            <button class="btn_bulk_translate btn btn-sm btn-primary ml-3 disabled <?php echo ($demo) ? 'disabled_d' : ''; ?>"><?php echo _("translate missing values"); ?>&nbsp;&nbsp;<i style="vertical-align:middle" class="fas fa-globe"></i></button>
                    ...
                    <div class="card-body">
                        <?php
                        $query_s = "SELECT * FROM svt_poi_staging WHERE id_poi IN (SELECT id FROM svt_pois WHERE id_room IN (SELECT id FROM svt_rooms WHERE id_virtualtour=$id_virtualtour_sel));";
                        ...
                        if($result_s) {
                            if($result_s->num_rows > 0) {
                                while($row_s = $result_s->fetch_array(MYSQLI_ASSOC)) {
                                    $staging = $row_s;
                                    $id_staging = $row_s['id'];
                                    $array_input_lang_s = array();
                                    $query_lang_s = "SELECT * FROM svt_poi_staging_lang WHERE id_staging=$id_staging;";
                                    $result_lang_s = $mysqli->query($query_lang_s);
                                    if($result_lang_s) {
                                        if ($result_lang_s->num_rows > 0) {
                                            while($row_lang_s = $result_lang_s->fetch_array(MYSQLI_ASSOC)) {
                                                $language = $row_lang_s['language'];
                                                unset($row_lang_s['id_staging']);
                                                unset($row_lang_s['language']);
                                                $array_input_lang_s[$language]=$row_lang_s;
                                            }
                                        }
                                    }
                                    echo print_value_input('tooltip',$staging,$array_input_lang_s,'svt_poi_staging_lang','text');
                                }
                            }
                        }
                        ?>
...
<script>
    (function($) {
        ...
        $(document).ready(function () {
            ...
            window.lang_editors['svt_virtualtours_lang'] = [];
            window.lang_editors['svt_virtualtours_lang']['learning_modal_description'] = [];
            $('div[data-table="svt_virtualtours_lang"][data-field="learning_modal_description"]').each(function() {
                var elem = $(this);
                var lang = elem.attr('data-lang');
                if(lang===undefined) lang = "orig";
                var id = elem.attr('data-id');
                window.lang_editors['svt_virtualtours_lang']['learning_modal_description'][lang+'-'+id] = new Quill(elem[0], {
                    modules: {
                ...
                if(lang!=='orig') {
                    window.lang_editors['svt_virtualtours_lang']['learning_modal_description'][lang+'-'+id].on('text-change', function (delta, oldDelta, source) {
                        elem.trigger('input');
                    });
                }
            });
