...
$head_php_array = json_decode($_POST['head_php_array'],true);
...
$buy_services = (int)$_POST['buy_services'];
...
$mail_service_purchased_subject = $_POST['mail_service_purchased_subject'];
$mail_service_purchased_body = $_POST['mail_service_purchased_body'];
...
$notify_service_purchase = (int)$_POST['notify_service_purchase'];
$notify_vt_create = (int)$_POST['notify_vt_create'];
$notify_custom_domain_request = (int)$_POST['notify_custom_domain_request'];
...
if($smt = $mysqli->prepare($query)) {
    $smt->bind_param('sssssssssssssisisssssssssiiiiiiiiiiisiissssssssssssssiiiiiiiiiiiiiiiiiiiissssssssssssssiissssssssssssissiiiiiiiiisiiiissssisssssisiiiisssssisiississssissssisiiisiiissisi',
        ...
        $enable_custom_domain, $custom_domain_ip_address, $notify_custom_domain_request,$buy_services,
        $notify_service_purchase,$mail_service_purchased_subject,$mail_service_purchased_body,$tour_inactive_days,$tawkto_chat_link,$pwa_enable);
    $result = $smt->execute();
}
...
foreach ($head_php_array as $name=>$content) {
    if($name=='custom_b') {
        $url_head = __DIR__.DIRECTORY_SEPARATOR.'..'.DIRECTORY_SEPARATOR.'..'.DIRECTORY_SEPARATOR.'backend'.DIRECTORY_SEPARATOR.'header_php'.DIRECTORY_SEPARATOR.'custom_b.php';
    } else {
        $url_head = __DIR__.DIRECTORY_SEPARATOR.'..'.DIRECTORY_SEPARATOR.'..'.DIRECTORY_SEPARATOR.'viewer'.DIRECTORY_SEPARATOR.'header_php'.DIRECTORY_SEPARATOR.$name.'.php';
    }
    if(file_exists($url_head) && $content=='') {
        @unlink($url_head);
    } else {
        if($content!='') {
            @file_put_contents($url_head,'<?php'.PHP_EOL.$content);
        }
    }
}
...
if($result) {
    save_input_langs($array_lang,'svt_settings_lang','id_settings',1);
