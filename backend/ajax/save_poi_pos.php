...
$query = "UPDATE svt_pois SET yaw=?,pitch=?,size_scale=?,rotateX=?,rotateZ=?,embed_coords=?,embed_size=?,transform3d=?,scale=?,zIndex=?,params=?,embed_params=?,visible_multiview_ids=?,sticky=? WHERE id=?;";
if($smt = $mysqli->prepare($query)) {
    $smt->bind_param('dddiissiiisssii',$yaw,$pitch,$size_scale,$rotateX,$rotateZ,$embed_coords,$embed_size,$transform3d,$scale,$zindex,$params,$embed_params,$visible_multiview_ids,$sticky,$id_poi);
    $result = $smt->execute();
    if ($result) {
        ob_end_clean();
