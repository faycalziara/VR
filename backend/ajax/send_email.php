...
switch ($_POST['type']) {
    ...
    case 'custom_domain_remove':
        if(!$settings['notify_custom_domain_request']) {
            ob_end_clean();
            echo json_encode(array("status"=>"ok"));
            exit;
        }
        ...
        $id_custom_domain = $_POST['id_custom_domain'];
        if(!empty($settings['notify_email'])) {
            $email = $settings['notify_email'];
        } else {
            $email = $settings['smtp_from_email'];
        ...
        switch ($_POST['type']) {
            case 'custom_domain_request':
                $subject = _("New Custom Domain Request");
                break;
            case 'custom_domain_remove':
                $subject = _("Remove Custom Domain Request");
                break;
        }
        $query = "SELECT u.username,u.email,p.name as plan FROM svt_users as u LEFT JOIN svt_plans as p ON p.id=u.id_plan WHERE u.id=$id_user LIMIT 1;";
        $result = $mysqli->query($query);
        ...
        $query = "SELECT custom_domain FROM svt_custom_domains WHERE id=$id_custom_domain;";
        $result = $mysqli->query($query);
        if($result) {
            if($result->num_rows==1) {
                $row = $result->fetch_array(MYSQLI_ASSOC);
        ...
        $body = _("Username").": $username<br>"._("E-Mail").": $email_u<br>"._("Plan").": $plan<br>"._("Custom Domain").": $custom_domain";
        $subject_q = str_replace("'","\'",$subject);
        $body_q = str_replace("'","\'",$body);
        $result_ins = $mysqli->query("INSERT INTO svt_notifications(id_user,subject,body,notified) VALUES($id_user,'$subject_q','$body_q',1);");
        if($result_ins) {
