...
$query = "SELECT image,icon FROM svt_poi_staging WHERE id_poi IN (SELECT id FROM svt_pois WHERE id_room IN (SELECT id FROM svt_rooms WHERE id_virtualtour IN (SELECT id FROM svt_virtualtours WHERE aws_s3=0)));";
$result = $mysqli->query($query);
if($result) {
    if($result->num_rows>0) {
        while($row=$result->fetch_array(MYSQLI_ASSOC)) {
            ...
            $icon = $row['icon'];
            if($image!='') {
                if(!in_array($image,$array_staging_files)) {
                    array_push($array_staging_files,$image);
                }
            }
            if($icon!='') {
                if(!in_array($icon,$array_staging_files)) {
                    array_push($array_staging_files,$icon);
                }
            }
        }
    }
...
check_files($path."viewer".DIRECTORY_SEPARATOR."panoramas".DIRECTORY_SEPARATOR."stereo".DIRECTORY_SEPARATOR,$array_rooms_files);
...
check_files($path."viewer".DIRECTORY_SEPARATOR."staging".DIRECTORY_SEPARATOR,$array_staging_files);
...
if($result) {
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_array(MYSQLI_ASSOC)) {
            $id_vt = $row['id'];
            array_push($array_id_vt_all, $id_vt);
        }
    }
}
$dir = $path . "vr" . DIRECTORY_SEPARATOR . "img" . DIRECTORY_SEPARATOR;
$files = scandir($dir);
foreach ($files as $file) {
    if (is_dir($dir . DIRECTORY_SEPARATOR . $file) && $file != '.' && $file != '..' && $file != 'custom') {
        if(!in_array($file,$array_id_vt_all)) {
            deleteDir($dir . DIRECTORY_SEPARATOR . $file);
        }
    }
}
