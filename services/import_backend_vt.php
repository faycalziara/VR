...
if($sample) {
    ...
    $file_zip = dirname(__FILE__)."/import_tmp/".urldecode($_POST['file_name']);
}
...
if(isset($commands_import[0]['fields']['id_room_initial'])) {
    $id_room_initial = $commands_import[0]['fields']['id_room_initial'];
    $query = str_replace('%id_room_initial%',"-2",$query);
}
...
$mysqli->close();
$path_dest = realpath(dirname(__FILE__) . '/..');
if(file_exists($path_import.DIRECTORY_SEPARATOR.'vr'.DIRECTORY_SEPARATOR.'img'.DIRECTORY_SEPARATOR)) {
    recursive_copy($path_import.DIRECTORY_SEPARATOR.'vr'.DIRECTORY_SEPARATOR.'img'.DIRECTORY_SEPARATOR,$path_dest.DIRECTORY_SEPARATOR.'vr'.DIRECTORY_SEPARATOR.'img'.DIRECTORY_SEPARATOR.$id_vt.DIRECTORY_SEPARATOR);
}
...
$mysqli->query("SET NAMES 'utf8mb4';");
$commands_import = get_commands($commands,'svt_virtualtours_versions');
foreach ($commands_import as $command_import) {
    $id_icon = $command_import['fields']['id'];
    $query = $command_import['sql'];
    if($version_diff) $query=fix_query($query,'svt_virtualtours_versions');
    $query = str_replace("%id_virtualtour%",$id_vt,$query);
    $query = query_mapping($query,$array_mapping);
    if($debug) {
        $date = date('Y-m-d H:i');
...
if($id_room_initial>0) {
    $id_room_initial_new = $rooms_mapping[$id_room_initial];
    $mysqli->query("UPDATE svt_virtualtours SET id_room_initial=$id_room_initial_new WHERE id=$id_vt;");
}
$commands_import = get_commands($commands,'svt_maps');
foreach ($commands_import as $command_import) {
    $id_map = $command_import['fields']['id'];
...
$poi_gallery_mapping = array();
foreach ($commands_import as $command_import) {
    $id_poi_gallery = $command_import['fields']['id'];
    $id_poi = $command_import['fields']['id_poi'];
    $query = $command_import['sql'];
    ...
    if(!$result) {
        ...
        $poi_gallery_mapping[$id_poi_gallery]=$id_poi_gallery_new;
    }
}
$mysqli->close();
$mysqli = new mysqli(DATABASE_HOST, DATABASE_USERNAME, DATABASE_PASSWORD, DATABASE_NAME);
...
$mysqli->query("SET NAMES 'utf8mb4';");
$commands_import = get_commands($commands,'svt_poi_gallery_lang');
foreach ($commands_import as $command_import) {
    $id_poi_gallery = $command_import['fields']['id_poi_gallery'];
    $query = $command_import['sql'];
    if($version_diff) $query=fix_query($query,'svt_poi_gallery_lang');
    $query = str_replace("%id_poi_gallery%",$poi_gallery_mapping[$id_poi_gallery],$query);
    $query = query_mapping($query,$array_mapping);
    if($debug) {
        $date = date('Y-m-d H:i');
        file_put_contents(realpath(dirname(__FILE__))."/log_import.txt","$date - query: $query".PHP_EOL,FILE_APPEND);
...
$mysqli->query("SET NAMES 'utf8mb4';");
$commands_import = get_commands($commands,'svt_poi_staging');
foreach ($commands_import as $command_import) {
    $id_poi = $command_import['fields']['id_poi'];
    $id_staging = $command_import['fields']['id'];
    $query = $command_import['sql'];
    if($version_diff) $query=fix_query($query,'svt_poi_staging');
    if(!empty($id_poi) && $id_poi!=0) {
        $id_poi_new = $pois_mapping[$id_poi];
    } else {
        $id_poi_new='NULL';
    ...
    $result=$mysqli->query($query);
    if($result) {
        $id_staging_new = $mysqli->insert_id;
        $staging_mapping[$id_staging]=$id_staging_new;
    } else {
        if($debug) {
            $date = date('Y-m-d H:i');
            file_put_contents(realpath(dirname(__FILE__))."/log_import.txt","$date - error: ".$mysqli->error.PHP_EOL,FILE_APPEND);
...
$mysqli->query("SET NAMES 'utf8mb4';");
$commands_import = get_commands($commands,'svt_poi_staging_lang');
foreach ($commands_import as $command_import) {
    $id_staging = $command_import['fields']['id_staging'];
    $query = $command_import['sql'];
    if($version_diff) $query=fix_query($query,'svt_poi_staging_lang');
    $query = str_replace("%id_staging%",$staging_mapping[$id_staging],$query);
    $query = query_mapping($query,$array_mapping);
    if($debug) {
        $date = date('Y-m-d H:i');
        file_put_contents(realpath(dirname(__FILE__))."/log_import.txt","$date - query: $query".PHP_EOL,FILE_APPEND);
